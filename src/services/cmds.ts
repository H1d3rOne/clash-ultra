...
// Get runtime config controlled by the app
...
export async function getUltraConfig() {
  return invoke<IVergeConfig>('get_ultra_config')
}

export async function patchUltraConfig(payload: IVergeConfig) {
  return invoke<void>('patch_ultra_config', { payload })
}
...
export interface ProxySpeedTestBatchItem {
  proxyName: string
  result?: ProxySpeedTestResult | null
  error?: string | null
}
...
export interface ProxyDelayTestBatchItem {
  proxyName: string
  result?: ProxyDelayTestResult | null
  error?: string | null
}

export async function cmdTestProxyDelay(
  proxyName: string,
  url?: string,
  timeout = 10000,
  profileUid?: string,
) {
  return invoke<ProxyDelayTestResult>('test_proxy_delay', {
    proxyName,
    url,
    timeout,
    profileUid,
...
export async function cmdTestProxyDelayBatch(
  proxyNames: string[],
  url?: string,
  timeout = 10000,
  concurrency = 10,
  ...
  return invoke<ProxyDelayTestBatchItem[]>('test_proxy_delay_batch', {
    proxyNames,
    url,
    timeout,
    concurrency,
...
export async function cmdTestProxySpeed(
  ...
  return invoke<ProxySpeedTestResult>('test_proxy_speed', {
    proxyName,
    url,
    timeout,
    maxBytes,
...
export async function cmdTestProxySpeedBatch(
  proxyNames: string[],
  url?: string,
  timeout = 15000,
  maxBytes = 20 * 1024 * 1024,
  ...
  return invoke<ProxySpeedTestBatchItem[]>('test_proxy_speed_batch', {
    proxyNames,
    url,
    timeout,
    maxBytes,
...
export async function getSystemUsage() {
  return invoke<number>('get_system_usage')
}
...
export const isServiceAvailable = async () => {
  try {
    ...
    // 未安装/未运行系统服务是 Sidecar 模式下的正常状态，不应在开发控制台反复刷 error。
    console.debug('Service check unavailable:', error)
    return false
  }
}
