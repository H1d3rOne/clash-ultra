...
const resolveMenuOrder = <T extends { path: string }>(
  ...
  for (const path of defaultOrder) {
    ...
    const defaultIndex = defaultOrder.indexOf(path)
    const previousDefaultPath = defaultOrder
      .slice(0, defaultIndex)
      .reverse()
      .find((item) => seen.has(item))
    const nextDefaultPath = defaultOrder
      .slice(defaultIndex + 1)
      .find((item) => seen.has(item))

    if (previousDefaultPath) {
      resolved.splice(resolved.indexOf(previousDefaultPath) + 1, 0, path)
    } else if (nextDefaultPath) {
      resolved.splice(resolved.indexOf(nextDefaultPath), 0, path)
    } else {
      resolved.push(path)
    }
