...
interface IVergePortProxy {
  id?: string
  enabled?: boolean
  name?: string
  type?: 'mixed' | 'http' | 'socks' | string
  listen?: string
  port?: number
  /** 端口代理自己的路由策略；只影响当前 listener，不修改 Mihomo 顶层 mode */
  routeMode?: 'rule' | 'global' | 'direct' | string
  ...
  /** 端口代理自己的代理组选择状态，name/now 使用运行时端口级名称 */
...
interface IVergeConfig {
  ...
  enable_ui_lightweight_optimizations?: boolean
  ...
  default_speed_test?: string
  default_speed_test_concurrency?: number
  default_speed_test_timeout?: number
  ...
  port_proxies?: IVergePortProxy[]
  enabled_profile_uids?: string[]
  /** 系统代理入口选择的运行订阅 UID */
  system_proxy_profile_uid?: string
  /** 虚拟网卡代理入口选择的运行订阅 UID */
  tun_proxy_profile_uid?: string
  rule_template_items?: IRuleTemplateItem[]
  /** 无规则 remote/local 订阅的默认规则模板，支持 __ALL_PROXIES__ 和 __ALL_PROXY_PROVIDERS__ 占位符 */
  default_rule_template?: string
  /** 当前选中的默认规则模板 key，用于区分内容相同的内置/自定义模板 */
  default_rule_template_key?: string
  webdav_url?: string
