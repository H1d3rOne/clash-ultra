...
export const useTrafficMonitorEnhanced = (options?: {
  ...
  const clientRef = useRef<TrafficWorkerClient | null>(null)
  ...
  const appendData = useCallback(
    (traffic: Traffic) => {
      ...
      // 只有图表类消费者真实挂载时才维护采样 Worker/历史缓冲。
      // Dock 等轻量速率展示会继续使用原始 WebSocket 数据，但不再在
      // 非首页/无图表场景下额外拉起 Worker，避免空闲时长期占用 WebKit 内存。
      if (refCounter.getCount() === 0) return

      const client = getWorkerClient()
      clientRef.current = client
      client.appendData(traffic)
    },
    [enabled],
  )
 ...
 * 图表数据Hook
 */
export const useTrafficGraphDataEnhanced = (options?: {
  enabled?: boolean
}) => {
  const { graphData, samplerStats, referenceCount } = useTrafficMonitorEnhanced(
