...
type WebSocketChannelMessage = Message | RawTextChannelMessage

const textDecoder = new TextDecoder()

const decodeBytes = (bytes: Uint8Array) => textDecoder.decode(bytes)

const normalizeWsTextMessage = (msg: WebSocketChannelMessage) => {
  if (typeof msg === 'string') return msg
  if (msg instanceof ArrayBuffer) return decodeBytes(new Uint8Array(msg))
  if (Array.isArray(msg)) return decodeBytes(new Uint8Array(msg))
  if (msg instanceof Uint8Array) return decodeBytes(msg)
  ...
  if (payload.type === 'Binary' && Array.isArray(payload.data)) {
    return decodeBytes(new Uint8Array(payload.data))
  }
  ...
  entry.connectWs = async () => {
    ...
    try {
      ...
      ws.addListener((msg: Message) => {
        const text = normalizeWsTextMessage(msg)
        if (text === null) return
        const activeOwner = pickActiveOwner(entry)
        if (!activeOwner) return

        activeOwner.handleMessage(text)
      })
    } catch (ignoreError) {
