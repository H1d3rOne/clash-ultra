...
export function LiteModeViewer({ ref }: { ref?: Ref<DialogRef> }) {
  ...
  const [values, setValues] = useState({
    ...
    uiLightweightOptimizations: true,
  })
  ...
  const onSave = useLockFn(async () => {
    try {
      await patchVerge({
        ...
        enable_ui_lightweight_optimizations: values.uiLightweightOptimizations,
      })
      setOpen(false)
    } catch (err) {
  ...
  return (
    ...
    >
      <List>
        ...
        <ListItem sx={{ padding: '5px 2px' }}>
          <ListItemText
            primary={t('settings.modals.liteMode.toggles.uiOptimizations')}
            secondary={t(
              'settings.modals.liteMode.messages.uiOptimizationsHint',
            )}
            sx={{ maxWidth: 'fit-content' }}
          />
          <TooltipIcon
            title={t('settings.modals.liteMode.tooltips.uiOptimizations')}
            sx={{ opacity: '0.7' }}
          />
          <Switch
            edge="end"
            checked={values.uiLightweightOptimizations}
            onChange={(_, c) =>
              setValues((v) => ({ ...v, uiLightweightOptimizations: c }))
            }
            sx={{ marginLeft: 'auto' }}
          />
        </ListItem>
