...
interface ProxyChainProps {
  ...
  onConnectChain?: () => Promise<void> | void
  onDisconnectChain?: () => Promise<void> | void
  onClearChain?: () => Promise<void> | void
}
...
export const ProxyChain = ({
  ...
  const effectiveIsConnected = isConnectedOverride ?? isConnected
  const hasCustomConnectHandler = Boolean(onConnectChain || onDisconnectChain)
  ...
  const handleConnect = useCallback(async () => {
    if (hasCustomConnectHandler) {
      if (!effectiveIsConnected && proxyChain.length < 2) {
      ...
      try {
        if (effectiveIsConnected) {
          await onDisconnectChain?.()
        } else {
          await onConnectChain?.()
        }
      } catch (error) {
        console.error('Failed to update proxy chain:', error)
        alert(
  ...
  // 定时更新延迟数据
  useEffect(() => {
    if ((lightweightOptimizations && !pageVisible) || !proxies?.records) return
  ...
  }, [lightweightOptimizations, pageVisible, proxies?.records]) // 只依赖轻量优化、可见性和 proxies.records
      ...
      >
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <Button
            size="small"
            variant="contained"
            startIcon={effectiveIsConnected ? <LinkOff /> : <Link />}
            onClick={handleConnect}
            disabled={
              isConnecting ||
              proxyChain.length < 2 ||
              connectDisabled ||
              (!hasCustomConnectHandler && mode !== 'global' && !selectedGroup)
            }
            color={effectiveIsConnected ? 'error' : 'success'}
            sx={{
              minWidth: 90,
            }}
            title={
