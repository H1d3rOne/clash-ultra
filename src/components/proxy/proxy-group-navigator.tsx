...
export const ProxyGroupNavigator = ({
  ...
  const processedGroups = useMemo(() => {
    return proxyGroupNames
      ...
      .map((name) => {
        const displayName = getGroupDisplayName?.(name) || name
        return {
          name,
          displayName,
          displayChar: getGroupDisplayChar(displayName),
        }
      })
  }, [getGroupDisplayName, proxyGroupNames])
  ...
  return (
    ...
    >
      {processedGroups.map(({ name, displayName, displayChar }) => (
        <Tooltip key={name} title={displayName} placement="left" arrow>
          <Button
            size="small"
            variant="text"
