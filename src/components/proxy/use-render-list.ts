...
export const useRenderList = (
  ...
  const effectiveProxiesData = useMemo(() => {
    if (!overrideGroups) return proxiesData
    const groups = overrideGroups.filter(
      (group): group is IProxyGroupItem =>
        Boolean(group?.name) && Array.isArray(group.all),
    )
    return {
      groups,
      global: groups[0],
      proxies: groups.flatMap((group) => group.all ?? []),
    }
  }, [overrideGroups, proxiesData])

  // 确保代理数据加载
  useEffect(() => {
    if (overrideGroups) return
  ...
  }, [overrideGroups, proxiesData, mode, refreshProxy])
  ...
  const renderList: IRenderItem[] = useMemo(() => {
    if (!effectiveProxiesData) return []

    // 链式代理模式下，显示代理组和其节点
    if (isChainMode && runtimeConfig && mode === 'rule') {
      // 使用正常的规则模式代理组
      const allGroups = effectiveProxiesData.groups.length
        ? effectiveProxiesData.groups
        : [effectiveProxiesData.global!]
    ...
    const renderGroups = (
      useRule && effectiveProxiesData.groups.length
        ? effectiveProxiesData.groups
        : effectiveProxiesData.global
          ? [effectiveProxiesData.global]
          : []
    ).filter((group: ProxyGroup | undefined): group is ProxyGroup =>
      Boolean(group?.name),
    )
