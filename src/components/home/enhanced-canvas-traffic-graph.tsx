...
export const EnhancedCanvasTrafficGraph = memo(
  function EnhancedCanvasTrafficGraph({
    ...
    const { dataPoints, requestRange, samplerStats } =
      useTrafficGraphDataEnhanced({ enabled: pageVisible })
    ...
    }, [pageVisible, requestRange, timeRange])
    ...
    const releaseCanvasBitmap = useCallback(
      (canvas: HTMLCanvasElement | null) => {
        if (!canvas) return
        canvas.width = 1
        canvas.height = 1
    ...
    }, [pageVisible, pause_render_traffic_stats_on_blur])
    ...
    useEffect(() => {
      ...
      return () => {
        ...
        releaseCanvasBitmap(hoverCanvas)
      }
    }, [releaseCanvasBitmap])
