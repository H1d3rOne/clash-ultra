 ...
 * - page sets `window.__ULTRA_ENABLE_DEBUG_LOGS__ = true`
 * - localStorage item `ULTRA_DEBUG_LOGS` is truthy (1/true/yes)
...
const readGlobalFlag = (): boolean | null => {
  ...
  const flag = (window as any).__ULTRA_ENABLE_DEBUG_LOGS__
...
const readStoredFlag = (): boolean | null => {
  if (typeof window === 'undefined') return null
  try {
    const stored = window.localStorage?.getItem('ULTRA_DEBUG_LOGS')
