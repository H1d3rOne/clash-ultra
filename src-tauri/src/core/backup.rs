        ...
        let client = reqwest_dav::ClientBuilder::new()
            .set_agent(
                reqwest::Client::builder()
                    ...
                    .user_agent(format!("clash-ultra/{APP_VERSION} ({OS} WebDAV-Client)"))
...
pub async fn create_backup() -> Result<(String, PathBuf), Error> {
    ...
    let verge_text = fs::read_to_string(dirs::app_config_path()?).await?;
    ...
    zip.start_file(dirs::APP_CONFIG, options)?;
