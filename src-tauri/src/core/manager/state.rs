...
impl CoreManager {
    ...
    pub(super) async fn start_core_by_sidecar(&self) -> Result<()> {
        ...
        let clash_core = Config::app_config().await.latest_arc().get_valid_clash_core();
        let config_dir = dirs::app_home_dir()?;

        #[cfg(unix)]
        let previous_mask = unsafe { tauri_plugin_clash_ultra_sysinfo::libc::umask(0o007) };
        ...
        #[cfg(unix)]
        unsafe {
            tauri_plugin_clash_ultra_sysinfo::libc::umask(previous_mask)
        };
