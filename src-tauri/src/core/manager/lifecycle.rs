...
use tauri_plugin_clash_ultra_sysinfo;
...
impl CoreManager {
    ...
    pub async fn change_core(&self, clash_core: &String) -> Result<(), String> {
        ...
        let verge_data = Config::app_config().await.latest_arc();
    ...
    fn after_core_process(&self) {
        ...
        tauri_plugin_clash_ultra_sysinfo::set_app_core_mode(app_handle, self.get_running_mode().to_string());
    }
    ...
    async fn wait_for_service_if_needed(&self) {
        ...
        let needs_service = Config::app_config().await.latest_arc().enable_tun_mode.unwrap_or(false);
