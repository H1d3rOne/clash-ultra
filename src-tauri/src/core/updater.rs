...
impl SilentUpdater {
    ...
    async fn ask_user_to_install(app_handle: &tauri::AppHandle, version: &str) -> bool {
        ...
        let title = clash_ultra_i18n::t!("notifications.updateReady.title");
        let body = clash_ultra_i18n::t!("notifications.updateReady.body").replace("{version}", version);
        let install_now = clash_ultra_i18n::t!("notifications.updateReady.installNow").into_owned();
        let later = clash_ultra_i18n::t!("notifications.updateReady.later").into_owned();
...
impl SilentUpdater {
    async fn check_and_download(&self, app_handle: &tauri::AppHandle) -> Result<()> {
        ...
        let auto_check = Config::app_config()
            .await
            .latest_arc()
            .auto_check_update
            .unwrap_or(true);
