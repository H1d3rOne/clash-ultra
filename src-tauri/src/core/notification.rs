...
impl NotificationSystem {
    ...
    fn serialize_event(event: FrontendEvent) -> (&'static str, Result<serde_json::Value, serde_json::Error>) {
        match event {
            FrontendEvent::RefreshClash => ("ultra://refresh-clash-config", Ok(json!("yes"))),
            FrontendEvent::RefreshUltraConfig => ("ultra://refresh-ultra-config", Ok(json!("yes"))),
            FrontendEvent::NoticeMessage { status, message } => {
                ("ultra://notice-message", serde_json::to_value((status, message)))
            }
            FrontendEvent::ProfileChanged { current_profile_id } => ("profile-changed", Ok(json!(current_profile_id))),
            FrontendEvent::TimerUpdated { profile_index } => ("ultra://timer-updated", Ok(json!(profile_index))),
