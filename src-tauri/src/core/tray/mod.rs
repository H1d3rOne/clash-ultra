...
use clash_ultra_limiter::{Limiter, SystemClock, SystemLimiter};
use clash_ultra_logging::logging_error;
...
pub const TRAY_ID: &str = "clash-ultra-tray";
...
impl Tray {
    ...
    pub async fn update_click_behavior(&self) -> Result<()> {
        ...
        let app_handle = handle::Handle::app_handle();
        let tray_event = { Config::app_config().await.latest_arc().tray_event.clone() };
    ...
    pub async fn update_tooltip(&self) -> Result<()> {
        ...
        let sys_proxy_text = clash_ultra_i18n::t!("tray.tooltip.systemProxy");
        let tun_text = clash_ultra_i18n::t!("tray.tooltip.tun");
        let profile_text = clash_ultra_i18n::t!("tray.tooltip.profile");
    ...
    pub async fn update_part(&self) -> Result<()> {
        ...
        let verge = Config::app_config().await.data_arc();
...
async fn create_tray_menu(
    ...
    let verge_settings = Config::app_config().await.latest_arc();
    ...
    let outbound_modes = if show_outbound_modes_inline {
        ...
        let current_mode_text = match current_proxy_mode {
            "global" => clash_ultra_i18n::t!("tray.global"),
            "direct" => clash_ultra_i18n::t!("tray.direct"),
            _ => clash_ultra_i18n::t!("tray.rule"),
        };
    ...
    let app_version = &MenuItem::with_id(
        app_handle,
        MenuIds::APP_VERSION,
        format!("{} {version}", texts.app_version),
        true,
        None::<&str>,
    )?;
