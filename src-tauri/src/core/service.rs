...
use clash_ultra_service_ipc::CoreConfig;
...
fn uninstall_service() -> Result<()> {
    ...
    let uninstall_path = binary_path.with_file_name("clash-ultra-service-uninstall.exe");
...
fn install_service() -> Result<()> {
    ...
    let binary_path = dirs::service_path()?;
    let install_path = binary_path.with_file_name("clash-ultra-service-install.exe");
...
#[cfg(target_os = "linux")]
fn linux_service_binary(name: &str) -> Result<PathBuf> {
    let exe = tauri::utils::platform::current_exe()?;
    let exe_dir = exe
        .parent()
        .with_context(|| format!("failed to get parent directory for {exe:?}"))?;
    ...
    let mut discovered = std::fs::read_dir(exe_dir)?
    ...
    discovered.sort();
    if let Some(candidate) = discovered.into_iter().next() {
        return Ok(candidate);
    }
...
fn uninstall_service() -> Result<()> {
    ...
    let uninstall_path = linux_service_binary("clash-ultra-service-uninstall")?;
...
fn install_service() -> Result<()> {
    ...
    let install_path = linux_service_binary("clash-ultra-service-install")?;
...
fn linux_running_as_root() -> bool {
    ...
    use tauri_plugin_clash_ultra_sysinfo::is_current_app_handle_admin;
...
fn uninstall_service() -> Result<()> {
    ...
    let binary_path = dirs::service_path()?;
    let uninstall_path = binary_path.with_file_name("clash-ultra-service-uninstall");
    ...
    // clash_ultra_i18n::sync_locale(Config::app_config().await.latest_arc().language.as_deref());

    let prompt = clash_ultra_i18n::t!("service.adminUninstallPrompt");
    let command =
...
fn install_service() -> Result<()> {
    ...
    let binary_path = dirs::service_path()?;
    let install_path = binary_path.with_file_name("clash-ultra-service-install");
    ...
    let gid = tauri_plugin_clash_ultra_sysinfo::current_gid();
    let prompt = clash_ultra_i18n::t!("service.adminInstallPrompt");
    let command = format!(
        r#"do shell script "sudo CLASH_ULTRA_SERVICE_GID={gid} '{install_shell}'" with administrator privileges with prompt "{prompt}""#
    );
...
pub(super) async fn start_with_existing_service(config_file: &PathBuf) -> Result<()> {
    ...
    let app_config = Config::app_config().await;
    let clash_core = app_config.latest_arc().get_valid_clash_core();
    drop(app_config);

    let bin_ext = if cfg!(windows) { ".exe" } else { "" };
    let bin_path = current_exe()?.with_file_name(format!("{clash_core}{bin_ext}"));

    let payload = clash_ultra_service_ipc::ClashConfig {
    ...
    let response = clash_ultra_service_ipc::start_clash(&payload)
        .await
        .context("无法连接到Clash Ultra Service")?;
...
pub(super) async fn get_clash_logs_by_service() -> Result<Vec<CompactString>> {
    ...
    let response = clash_ultra_service_ipc::get_clash_logs()
        .await
        .context("无法连接到Clash Ultra Service")?;
...
pub(super) async fn stop_core_by_service() -> Result<()> {
    ...
    let response = clash_ultra_service_ipc::stop_clash()
        .await
        .context("无法连接到Clash Ultra Service")?;
...
pub async fn is_service_available() -> Result<()> {
    if let Err(e) = Path::metadata(clash_ultra_service_ipc::IPC_PATH.as_ref()) {
        let app_config = Config::app_config().await;
        let app_config_latest = app_config.latest_arc();
        let is_enable = app_config_latest.enable_tun_mode.unwrap_or(false);
        if is_enable {
            logging!(warn, Type::Service, "Some issue with service IPC Path: {}", e);
        }
        return Err(e.into());
    }
    clash_ultra_service_ipc::connect().await?;
    Ok(())
}
...
async fn wait_for_service_ipc(manager: &ServiceManager) -> Result<()> {
    ...
    let result = (|| async {
        ...
        clash_ultra_service_ipc::connect().await.map(drop)
    })
    .retry(backoff)
    .await;
...
pub fn is_service_ipc_path_exists() -> bool {
    Path::new(clash_ultra_service_ipc::IPC_PATH).exists()
}

impl ServiceManager {
    pub const fn config() -> clash_ultra_service_ipc::IpcConfig {
        clash_ultra_service_ipc::IpcConfig {
    ...
    pub async fn init(&self) -> Result<()> {
        if let Err(e) = clash_ultra_service_ipc::connect().await {
    ...
    pub async fn refresh(&self) -> Result<()> {
        self.run_operation(async {
            self.apply_service_status(if clash_ultra_service_ipc::is_reinstall_service_needed().await {
    ...
    async fn apply_service_status(&self, status: ServiceStatus) -> Result<()> {
        ...
        match status {
            ...
            ServiceStatus::InstallRequired => {
                ...
                wait_for_service_ipc(self).await?;
                if clash_ultra_service_ipc::is_reinstall_service_needed().await {
