...
pub async fn patch_clash(patch: &Mapping) -> Result<()> {
    let has_enabled_port_proxy = Config::app_config()
        .await
        .latest_arc()
        .port_proxies
        .as_ref()
        .is_some_and(|items| items.iter().any(|item| item.enabled.unwrap_or(false)));
    let should_force_rule_mode = has_enabled_port_proxy
        && patch
            .get("mode")
            .and_then(|value| value.as_str())
            .is_some_and(|mode| mode != "rule");
    let effective_patch = if should_force_rule_mode {
        let mut next = patch.clone();
        next.insert("mode".into(), "rule".into());
        Some(next)
    } else {
        None
    };
    let patch = effective_patch.as_ref().unwrap_or(patch);

    Config::clash().await.edit_draft(|d| d.patch_config(patch));

    let res = {
        let should_rebuild_port_proxy_listeners = patch.get("allow-lan").is_some()
            && Config::app_config()
                .await
                .latest_arc()
                .port_proxies
        ...
        } else if should_rebuild_port_proxy_listeners {
            // 端口代理由 listeners 实现。allow-lan 改变时需要重新生成这些 listener
            // 的 listen 地址，并重启核心确保旧监听地址被释放、新地址重新绑定。
            Config::generate().await?;
            CoreManager::global().restart_core().await?;
        } else {
...
fn determine_update_flags(patch: &IVerge, had_enabled_port_proxy: bool) -> UpdateFlags {
    ...
    let port_proxies = patch.port_proxies.as_ref();
    let enabled_profile_uids = patch.enabled_profile_uids.as_ref();
    let system_proxy_profile_uid = patch.system_proxy_profile_uid.as_ref();
    let tun_proxy_profile_uid = patch.tun_proxy_profile_uid.as_ref();
    ...
    if port_proxies.is_some()
        || enabled_profile_uids.is_some()
        || system_proxy_profile_uid.is_some()
        || tun_proxy_profile_uid.is_some()
    {
        update_flags.insert(UpdateFlags::CLASH_CONFIG);
    }
    if port_proxies.is_some() || (had_enabled_port_proxy && (system_proxy == Some(true) || tun_mode == Some(true))) {
        // Mihomo hot-reload does not always release removed listener sockets.
        // Port proxy switches must behave like the built-in port settings:
        // closing a proxy should immediately close its TCP listener, and
        // reopening it should bind the port again from a clean runtime state.
        // Switching to system proxy / TUN also disables port proxies in the
        // backend, so it must close the old listener sockets the same way.
        update_flags.insert(UpdateFlags::RESTART_CORE);
    }
    if system_proxy == Some(true) || tun_mode == Some(true) {
        // 开启系统代理/TUN 时会在后端兜底关闭端口代理，需要重新生成运行配置移除 listeners。
        update_flags.insert(UpdateFlags::CLASH_CONFIG);
    }
    if port_proxies.is_some_and(|items| items.iter().any(|item| item.enabled.unwrap_or(false))) {
        // 开启端口代理时会在后端兜底关闭系统代理/TUN，需要同步 OS 代理和托盘状态。
...
async fn process_terminated_flags(update_flags: UpdateFlags, patch: &IVerge) -> Result<()> {
    // 端口代理入口按 listener.rule/sub-rules 工作，开启时必须固定核心为 rule，
    // 避免托盘/快捷键/其它入口把核心切到 global/direct 后绕过端口规则。
    if patch
        .port_proxies
        .as_ref()
        .is_some_and(|items| items.iter().any(|item| item.enabled.unwrap_or(false)))
    {
        Config::clash().await.edit_draft(|d| {
            d.0.insert("mode".into(), "rule".into());
        });
    }
    ...
    if update_flags.contains(UpdateFlags::CLASH_CONFIG) && !update_flags.contains(UpdateFlags::RESTART_CORE) {
        CoreManager::global().update_config_checked().await?;
        handle::Handle::refresh_clash();
    }
    ...
    {
        clash_ultra_i18n::set_locale(language.as_str());
    }
    ...
    if update_flags.contains(UpdateFlags::SYSTRAY_ICON) {
        tray::Tray::global()
            .update_icon(&Config::app_config().await.latest_arc())
            .await?;
        #[cfg(target_os = "macos")]
...
fn normalize_exclusive_proxy_modes(draft: &mut IVerge, patch: &IVerge) {
    // 三种入口模式互斥：系统代理 / 端口代理 / TUN。这里做后端兜底，
    // 覆盖设置页、托盘、快捷键等所有入口，避免前端局部处理遗漏。
    if (patch.enable_system_proxy == Some(true) || patch.enable_tun_mode == Some(true))
        && let Some(port_proxies) = draft.port_proxies.as_mut()
    {
        for item in port_proxies {
            item.enabled = Some(false);
        }
    }

    let enabling_port_proxy = patch
        .port_proxies
        .as_ref()
        .is_some_and(|items| items.iter().any(|item| item.enabled.unwrap_or(false)));
    if enabling_port_proxy {
        draft.enable_system_proxy = Some(false);
        draft.enable_tun_mode = Some(false);
    }
}

pub async fn patch_app_config(patch: &IVerge, not_save_file: bool) -> Result<()> {
    let had_enabled_port_proxy = Config::app_config()
        .await
        .latest_arc()
        .port_proxies
        .as_ref()
    ...
    Config::app_config().await.edit_draft(|d| {
        d.patch_config(patch);
        normalize_exclusive_proxy_modes(d, patch);
    });

    let process_flag_result: std::result::Result<(), anyhow::Error> = {
        let update_flags = determine_update_flags(patch, had_enabled_port_proxy);
    ...
    if let Err(err) = process_flag_result {
        Config::app_config().await.discard();
        return Err(err);
    }
    Config::app_config().await.apply();
    logging_error!(Type::Backup, AutoBackupManager::global().refresh_settings().await);
    if !not_save_file {
        // 分离数据获取和异步调用
        let verge_data = Config::app_config().await.data_arc();
...
pub async fn fetch_app_config() -> Result<SharedDraft<IVerge>> {
    let draft = Config::app_config().await;
