use crate::{
    ...
    utils::{
        dirs::{PathBufExec as _, app_config_path, app_home_dir, local_backup_dir},
        help,
    },
};
...
/// Load restored ultra.yaml from disk, merge back WebDAV creds, save, and sync memory.
/// Also reload other restored configs so restarts won't overwrite them.
async fn finalize_restored_app_config(
    webdav_url: Option<String>,
    webdav_username: Option<String>,
    webdav_password: Option<String>,
) -> Result<()> {
    // Do NOT silently fallback to defaults; a broken/missing ultra.yaml means restore failed.
    // Propagate the error so the UI/user can react accordingly.
    let mut restored = help::read_yaml::<IVerge>(&app_config_path()?).await?;
    ...
    let verge_draft = Config::app_config().await;
    ...
    if let Err(err) = super::patch_app_config(&restored, true).await {
        logging!(error, Type::Backup, "Failed to apply restored app config: {err:#?}");
    }
    Ok(())
}
...
pub async fn restore_webdav_backup(filename: String) -> Result<()> {
    ...
    let res = finalize_restored_app_config(webdav_url, webdav_username, webdav_password).await;
...
pub async fn restore_local_backup(filename: String) -> Result<()> {
    ...
    let mut zip = zip::ZipArchive::new(file)?;
    zip.extract(app_home_dir()?)?;
    finalize_restored_app_config(webdav_url, webdav_username, webdav_password).await?;
    Ok(())
}
