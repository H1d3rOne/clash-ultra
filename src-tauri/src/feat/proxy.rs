...
pub async fn toggle_system_proxy() -> bool {
    ...
    let patch_result = super::patch_app_config(
        &IVerge {
...
pub async fn toggle_tun_mode(not_save_file: Option<bool>) -> bool {
    let current = Config::app_config().await.latest_arc().enable_tun_mode.unwrap_or(false);
    let enable = !current;

    match super::patch_app_config(
        &IVerge {
...
pub async fn copy_clash_env() {
    let env_ip = env::var("CLASH_ULTRA_IP").ok();
    let verge_cfg = Config::app_config().await.latest_arc();
    let ip = env_ip
        .as_deref()
