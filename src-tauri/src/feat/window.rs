...
pub async fn clean_async() -> bool {
    ...
    let proxy_task = tokio::task::spawn(async {
        let sys_proxy_enabled = Config::app_config()
            .await
            .data_arc()
            .enable_system_proxy
            .unwrap_or(false);
    ...
    let core_task = tokio::task::spawn(async {
        ...
        let tun_enabled = Config::app_config().await.data_arc().enable_tun_mode.unwrap_or(false);
        if tun_enabled {
...
pub async fn hide() {
    ...
    let enable_auto_light_weight_mode = Config::app_config()
        .await
        .data_arc()
