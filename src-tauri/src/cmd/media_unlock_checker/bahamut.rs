...
use super::{UnlockItem, UnlockProxyTarget, build_unlock_client};

pub(super) async fn check_bahamut_anime(client: &Client, proxy: Option<&UnlockProxyTarget>) -> UnlockItem {
    let cookie_store = Arc::new(Jar::default());

    let client_with_cookies = match build_unlock_client(
        proxy,
        Some(Arc::clone(&cookie_store)),
        Some(
