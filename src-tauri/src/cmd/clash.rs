...
) -> CmdResult<feat::ProxySpeedTestResult> {
    feat::test_proxy_speed(proxy_name, url, timeout, max_bytes, profile_uid)
        .await
        .stringify_err()
}
...
pub async fn test_proxy_speed_batch(
    ...
    profile_uid: Option<String>,
) -> CmdResult<Vec<feat::ProxySpeedTestBatchItem>> {
    feat::test_proxy_speed_batch(proxy_names, url, timeout, max_bytes, concurrency, profile_uid)
        .await
        .stringify_err()
}
...
pub async fn test_proxy_delay(
    ...
    timeout: Option<u64>,
    profile_uid: Option<String>,
) -> CmdResult<feat::ProxyDelayTestResult> {
    feat::test_proxy_delay(proxy_name, url, timeout, profile_uid)
        .await
        .stringify_err()
}
...
pub async fn test_proxy_delay_batch(
    ...
    concurrency: Option<usize>,
    profile_uid: Option<String>,
) -> CmdResult<Vec<feat::ProxyDelayTestBatchItem>> {
    feat::test_proxy_delay_batch(proxy_names, url, timeout, concurrency, profile_uid)
        .await
        .stringify_err()
}
