...
fn make_unique_profile_name(base: &str, existing_names: &[std::string::String]) -> std::string::String {
    let base = {
        let trimmed = base.trim();
        if trimmed.is_empty() { "Remote File" } else { trimmed }
    };

    let used = existing_names.iter().map(|name| name.as_str()).collect::<HashSet<_>>();
    ...
    unreachable!("profile name suffix search should always find a free name")
}

async fn ensure_imported_profile_name_unique(item: &mut PrfItem) {
    ...
    let base_name = item.name.as_ref().map(|name| name.as_str()).unwrap_or("Remote File");
    let unique_name = make_unique_profile_name(base_name, &existing_names);
    ...
    item.name = Some(unique_name.as_str().into());
}
...
pub async fn import_profile(url: std::string::String, option: Option<PrfOption>) -> CmdResult<String> {
    ...
    ensure_imported_profile_name_unique(item).await;
...
/// 创建一个新的配置文件
#[tauri::command]
pub async fn create_profile(item: PrfItem, file_data: Option<String>) -> CmdResult<String> {
    let item = &mut PrfItem::from(&item, file_data)
        .await
        .map_err(|err| format!("add profile error: {err}"))?;
    let uid = item
        .uid
        .clone()
        .ok_or_else(|| "add profile error: generated profile uid is empty".to_string())?;
