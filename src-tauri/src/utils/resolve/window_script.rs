pub fn build_window_initial_script(initial_theme_mode: &str, dark_background: &str, light_background: &str) -> String {
    ...
    format!(
        r#"
    window.__ULTRA_INITIAL_THEME_MODE = "{theme_mode}";
    window.__ULTRA_INITIAL_THEME_COLORS = {{
...
pub const WINDOW_INITIAL_SCRIPT: &str = r##"
    ...
    const initialColors = (() => {
        try {
            const colors = window.__ULTRA_INITIAL_THEME_COLORS;
    ...
    const initialThemeMode = typeof window.__ULTRA_INITIAL_THEME_MODE === "string"
        ? window.__ULTRA_INITIAL_THEME_MODE
        : "dark";
    ...
    const applyInitialTheme = (theme) => {
        ...
        try {
            localStorage.setItem("ultra-theme-mode-cache", theme);
        } catch (error) {
