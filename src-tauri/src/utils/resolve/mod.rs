...
pub fn resolve_setup_async() {
    AsyncHandler::spawn(|| async {
        logging!(info, Type::ClashUltra, "Version: {}", env!("CARGO_PKG_VERSION"));
...
pub(super) async fn init_hotkey() {
    ...
    let skip_register_hotkeys = !Config::app_config()
        .await
        .latest_arc()
...
pub fn init_signal() {
    ...
    clash_ultra_signal::register(feat::quit);
}
...
pub(super) async fn init_app_config() {
    logging_error!(Type::Setup, Config::init_config().await);
}

pub(super) async fn init_service_manager() {
    clash_ultra_service_ipc::set_config(Some(ServiceManager::config())).await;
...
pub(super) async fn init_window() {
    let is_silent_start = Config::app_config()
        .await
        .data_arc()
        .enable_silent_start
        .unwrap_or(false);
