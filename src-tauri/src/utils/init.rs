...
pub async fn delete_log() -> Result<()> {
    ...
    let auto_log_clean = {
        let verge = Config::app_config().await;
...
async fn init_dns_config() -> Result<()> {
    ...
    if !dns_path.exists() {
        ...
        help::save_yaml(&dns_path, &default_dns_config, Some("# Clash Ultra DNS Config")).await?;
    }
...
async fn initialize_config_files() -> Result<()> {
        ...
        && !path.exists()
    {
        ...
        help::save_yaml(&path, &template, Some("# Clash Ultra"))
            .await
            .map_err(|e| anyhow::anyhow!("Failed to create clash config: {}", e))?;
        logging!(info, Type::Setup, "Created clash config at {:?}", path);
    }

    if let Ok(path) = dirs::app_config_path()
        && !path.exists()
    {
        let template = IVerge::template();
        help::save_yaml(&path, &template, Some("# Clash Ultra"))
            .await
            .map_err(|e| anyhow::anyhow!("Failed to create app config: {}", e))?;
        logging!(info, Type::Setup, "Created app config at {:?}", path);
    }
    ...
    IVerge::validate_and_fix_config()
        .await
        .map_err(|e| anyhow::anyhow!("Failed to validate app config: {}", e))?;
...
pub fn init_scheme() -> Result<()> {
    ...
    for &scheme in DEEP_LINK_SCHEMES {
        let class_name = if scheme == "clash" { "Clash" } else { scheme };
        let base_key = format!("Software\\Classes\\{class_name}");
        let (scheme_key, _) = hkcu.create_subkey(&base_key)?;
        scheme_key.set_value("", &"Clash Ultra")?;
        scheme_key.set_value("URL Protocol", &"Clash Ultra URL Scheme Protocol")?;
...
#[cfg(target_os = "linux")]
pub fn init_scheme() -> Result<()> {
    const DESKTOP_FILE: &str = "clash-ultra.desktop";
...
const DEEP_LINK_SCHEMES: &[&str] = &["clash", "clash-ultra"];
