...
use clash_ultra_logging::{Type, logging_error};
...
fn use_script_sync(script: String, config: &Mapping, name: &String) -> Result<(Mapping, Vec<(String, String)>)> {
    ...
    let _ = context.register_global_builtin_callable("__ultra_log__".into(), 2, unsafe {
    ...
    let _ = context.eval(Source::from_bytes(
        ...
        log(data){__ultra_log__("log",JSON.stringify(data, null, 2))},
        info(data){__ultra_log__("info",JSON.stringify(data, null, 2))},
        error(data){__ultra_log__("error",JSON.stringify(data, null, 2))},
        debug(data){__ultra_log__("debug",JSON.stringify(data, null, 2))},
        warn(data){__ultra_log__("warn",JSON.stringify(data, null, 2))},
        table(data){__ultra_log__("table",JSON.stringify(data, null, 2))},
      });"#,
    ));
