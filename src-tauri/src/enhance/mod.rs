...
use crate::utils::{dirs, subscription::ensure_default_rule_template_with_template};
...
struct ConfigValues {
    ...
    default_rule_template: Option<String>,
    ...
    for (key, value) in clash_config.into_iter() {
        if key.as_str() == Some("tun") {
            ...
            if key.as_str() == Some("external-controller") {
                let enable_external_controller = Config::app_config()
                    .await
                    .latest_arc()
...
pub async fn enhance() -> Result<(Mapping, HashSet<String>, HashMap<String, ResultLog>)> {
    ...
    let (mut config, exists_keys, result_map) = process_profile_items(
        config,
        exists_keys,
        result_map,
        rules_item,
    ...
    ensure_default_rule_template_with_template(&mut config, default_rule_template.as_deref());
