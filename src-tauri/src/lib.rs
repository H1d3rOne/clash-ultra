...
mod app_init {
    ...
    pub fn setup_plugins(builder: tauri::Builder<tauri::Wry>) -> tauri::Builder<tauri::Wry> {
        ...
        let mut builder = builder
            .plugin(tauri_plugin_clash_ultra_sysinfo::init())
    ...
    /// Setup macOS application menu with an explicit display name.
    ///
    /// Tauri's default macOS app menu derives the first menu title from package
    /// metadata. After rebranding, relying on generated/cached metadata can leave
    /// the menu-bar application title showing the old upstream name, so keep the
    /// visible menu title and About metadata fixed to the new product name.
    #[cfg(target_os = "macos")]
    pub fn setup_macos_app_menu(app: &tauri::App) -> tauri::Result<()> {
        use tauri::menu::{AboutMetadata, HELP_SUBMENU_ID, Menu, PredefinedMenuItem, Submenu, WINDOW_SUBMENU_ID};

        const APP_NAME: &str = "Clash Ultra";
    ...
    pub fn generate_handlers() -> impl Fn(tauri::ipc::Invoke<tauri::Wry>) -> bool + Send + Sync + 'static {
        tauri::generate_handler![
            tauri_plugin_clash_ultra_sysinfo::commands::get_system_info,
            tauri_plugin_clash_ultra_sysinfo::commands::get_app_uptime,
            tauri_plugin_clash_ultra_sysinfo::commands::app_is_admin,
            tauri_plugin_clash_ultra_sysinfo::commands::export_diagnostic_info,
            tauri_plugin_clash_ultra_sysinfo::commands::get_system_usage,
            cmd::is_port_in_use,
            cmd::get_sys_proxy,
            cmd::get_auto_proxy,
            cmd::open_app_dir,
            ...
            cmd::get_clash_logs,
            cmd::get_ultra_config,
            cmd::patch_ultra_config,
            cmd::test_delay,
            cmd::test_proxy_delay,
            cmd::test_proxy_delay_batch,
            cmd::test_proxy_speed,
            cmd::test_proxy_speed_batch,
            cmd::get_app_dir,
            cmd::copy_icon_file,
...
pub fn run() {
    if let Err(e) = app_init::init_singleton_check() {
        eprintln!("Failed to initialize singleton check: {e:#}");
        return;
    }
    ...
    let builder = app_init::setup_plugins(tauri::Builder::default())
        .setup(|app| {
            ...
            #[cfg(target_os = "macos")]
            app_init::setup_macos_app_menu(app)?;
    ...
    mod event_handlers {
        ...
        pub fn handle_ready_resumed(_app_handle: &AppHandle) {
            ...
            if let Some(window) = _app_handle.get_webview_window("main") {
                let _ = window.set_title("Clash Ultra");
            }
        }
        ...
        pub fn handle_window_focus(focused: bool) {
            AsyncHandler::spawn(move || async move {
                let is_enable_global_hotkey = Config::app_config()
                    .await
                    .data_arc()
