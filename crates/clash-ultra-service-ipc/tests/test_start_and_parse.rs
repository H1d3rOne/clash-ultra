...
mod tests {
    ...
    async fn test_reinstall_service_needed() {
        #[cfg(unix)]
        {
            ...
            assert!(
                !clash_ultra_service_ipc::is_ipc_path_exists(),
                "IPC path should not exist after stopping the server"
            );

            let ipc_path = Path::new(clash_ultra_service_ipc::IPC_PATH);
            ...
            assert!(
                clash_ultra_service_ipc::is_reinstall_service_needed().await,
