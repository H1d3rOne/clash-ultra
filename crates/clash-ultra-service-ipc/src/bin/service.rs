//! Clash Ultra Service - Cross-platform IPC service daemon
//!
...
fn main() -> Result<()> {
    init_logger();
    if service_dispatcher::start("clash_ultra_service", ffi_service_main).is_err() {
...
fn run_service() -> platform_lib::Result<()> {
    ...
    let status_handle = service_control_handler::register("clash_ultra_service", event_handler)?;
...
async fn run_standalone() -> Result<()> {
    ...
    info!("Clash Ultra Service - Standalone Mode");
