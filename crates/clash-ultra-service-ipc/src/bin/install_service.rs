...
fn resolve_service_group_name() -> String {
    ...
    if let Some(gid) = env_u32("CLASH_ULTRA_SERVICE_GID")
...
fn main() -> Result<(), Error> {
    ...
    let service_binary_path = env::current_exe().unwrap().with_file_name("clash-ultra-service");

    if !service_binary_path.exists() {
        return Err(anyhow::anyhow!("clash-ultra-service binary not found"));
    }

    // 定义 bundle 路径
    let bundle_path = "/Library/PrivilegedHelperTools/app.clash-ultra.desktop.service.bundle";
    ...
    let target_binary_path = format!("{}/clash-ultra-service", macos_path);
    ...
    // 创建并写入 launchd plist
    let plist_file = "/Library/LaunchDaemons/app.clash-ultra.desktop.service.plist";
    ...
    let _ = run_command(
        "launchctl",
        &["enable", "system/app.clash-ultra.desktop.service"],
        debug,
    );
    ...
    let _ = run_command("launchctl", &["start", "app.clash-ultra.desktop.service"], debug);
...
#[cfg(target_os = "linux")]
fn main() -> Result<(), Error> {
    const SERVICE_NAME: &str = "clash-ultra-service";
    use std::env;
    use std::fs::File;
    use std::io::Write;
    use std::path::Path;
    ...
    let service_binary_path = resolve_linux_service_binary()?;
...
#[cfg(target_os = "linux")]
fn resolve_linux_service_binary() -> Result<std::path::PathBuf, Error> {
    let current_exe =
        std::env::current_exe().map_err(|e| anyhow::anyhow!("failed to get current executable: {}", e))?;
    let exe_dir = current_exe
        .parent()
        .ok_or_else(|| anyhow::anyhow!("failed to get executable directory"))?;

    let unsuffixed = exe_dir.join("clash-ultra-service");
    if unsuffixed.exists() {
        return Ok(unsuffixed);
    }

    // Linux Tauri externalBin inputs are often target-triple suffixed in the
    // build tree (for example `clash-ultra-service-x86_64-unknown-linux-gnu`).
    // Some packaging backends strip the suffix, some keep it. Be tolerant here
    // so the installer can find the actual service daemon in either layout.
    let mut suffixed_candidates = std::fs::read_dir(exe_dir)
        .map_err(|e| anyhow::anyhow!("failed to read executable directory: {}", e))?
        ...
        .filter(|path| {
            ...
            file_name.starts_with("clash-ultra-service-")
                && !file_name.starts_with("clash-ultra-service-install")
                && !file_name.starts_with("clash-ultra-service-uninstall")
        })
        .collect::<Vec<_>>();
    ...
    suffixed_candidates
        .into_iter()
        .next()
        .ok_or_else(|| anyhow::anyhow!("clash-ultra-service binary not found"))
}
...
fn main() -> anyhow::Result<()> {
    ...
    const SERVICE_NAME: &str = "clash_ultra_service";
    ...
    let service_binary_path = env::current_exe().unwrap().with_file_name("clash-ultra-service.exe");

    if !service_binary_path.exists() {
        eprintln!("clash-ultra-service.exe not found");
        std::process::exit(2);
    }

    let service_info = ServiceInfo {
        name: OsString::from(SERVICE_NAME),
        display_name: OsString::from("Clash Ultra Service"),
    ...
    service.set_description("Clash Ultra Service helps to launch Mihomo core")?;
    configure_windows_service_recovery(&service)?;
    service.start(&Vec::<&OsStr>::new())?;
