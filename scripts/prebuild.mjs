...
const TEMP_DIR = path.join(cwd, 'node_modules/.ultra')
...
function clashMetaAlpha() {
  ...
  return {
    name: 'ultra-mihomo-alpha',
    targetFile: `ultra-mihomo-alpha-${SIDECAR_HOST}${isWin ? '.exe' : ''}`,
...
function clashMeta() {
  ...
  return {
    name: 'ultra-mihomo',
    targetFile: `ultra-mihomo-${SIDECAR_HOST}${isWin ? '.exe' : ''}`,
    exeFile: `${name}${isWin ? '.exe' : ''}`,
...
function localSidecarExists(name) {
  const ext = platform === 'win32' ? '.exe' : ''
  const sidecarPath = path.join(SIDECAR_DIR, `${name}-${SIDECAR_HOST}${ext}`)
  return !FORCE && fs.existsSync(sidecarPath)
}
...
// =======================
const SERVICE_CRATE_DIR = path.join(cwd, 'crates', 'clash-ultra-service-ipc')
...
function buildLocalServiceBundle() {
  ...
  log_info(`Building local Clash Ultra service: cargo ${cargoArgs.join(' ')}`)
...
async function resolveServiceBundle() {
  if (!fs.existsSync(SERVICE_CRATE_DIR)) {
    throw new Error(`Service crate not found: ${SERVICE_CRATE_DIR}`)
  }

  buildLocalServiceBundle()
  await fsp.mkdir(SERVICE_DIR, { recursive: true })

  for (const name of SERVICE_BINARIES) {
    const { sourceFile, targetFile } = serviceFileInfo(name)
    const sourcePath = path.join(serviceBuildDir(), sourceFile)
    const targetPath = path.join(SERVICE_DIR, targetFile)

    if (!fs.existsSync(sourcePath)) {
      throw new Error(`Expected built service binary not found: ${sourcePath}`)
    }

    if (!(await hasFileChanged(sourcePath, targetPath))) {
      log_success(`service file unchanged, skipping copy: ${targetFile}`)
      continue
    }
...
const tasks = [
  {
    name: 'ultra-mihomo-alpha',
    func: async () => {
      if (localSidecarExists('ultra-mihomo-alpha')) {
        log_success('"ultra-mihomo-alpha" already exists, skipping version check')
        return
      }
  ...
  },
  {
    name: 'ultra-mihomo',
    func: async () => {
      if (localSidecarExists('ultra-mihomo')) {
        log_success('"ultra-mihomo" already exists, skipping version check')
        return
      }
