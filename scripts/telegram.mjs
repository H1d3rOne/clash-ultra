...
const CHAT_ID_RELEASE = '@clash_ultra_app' // 正式发布频道
const CHAT_ID_TEST = '@clash_ultra_test' // 测试频道

async function sendTelegramNotification() {
  if (!process.env.TELEGRAM_BOT_TOKEN) {
    log_info('TELEGRAM_BOT_TOKEN is not configured, skip Telegram notification')
    return
  }
  ...
  const downloadUrl =
    ...
    `https://github.com/H1d3rOne/clash-ultra/releases/download/v${version}`
  ...
  const content = `<b>🎉 <a href="https://github.com/H1d3rOne/clash-ultra/releases/tag/${releaseTag}">Clash Ultra v${version}</a> ${releaseTitle}</b>\n\n${formattedContent}`
    ...
    await axios.post(
      `https://api.telegram.org/bot${process.env.TELEGRAM_BOT_TOKEN}/sendMessage`,
      {
        ...
        link_preview_options: {
          is_disabled: false,
          url: `https://github.com/H1d3rOne/clash-ultra/releases/tag/v${encodedVersion}`,
